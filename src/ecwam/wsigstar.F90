...
!
      ...
      SUBROUTINE WSIGSTAR (KIJS, KIJL, WSWAVE, UFRIC, Z0M, CHRNCK, WSTAR, SIG_N)
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
      ...
      USE YOWPCONS , ONLY : G, ACDLIN, BCDLIN 
...
! ----------------------------------------------------------------------
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(IN) :: WSWAVE, UFRIC, Z0M, CHRNCK, WSTAR
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(OUT) :: SIG_N

      INTEGER(KIND=JWIM) :: IJ

      REAL(KIND=JWRB), PARAMETER :: BG_GUST = 0.0_JWRB ! NO BACKGROUND GUSTINESS WILL ONLY BE PARTIALLY USED IN FORECAST MODE (S0 12. IS NOT USED)
      ...
      REAL(KIND=JWRB) :: BCD_LOC, U10M1, ZN
...
! ----------------------------------------------------------------------
      ...
      IF (LLGCBZ0 .OR. LLNORMAGAM) THEN
        ...
        DO IJ=KIJS,KIJL
          ...
          ! CHARNOCK:
          ZCHAR=MAX(MIN(CHRNCK(IJ),ALPHAMAX),ALPHAMIN)
