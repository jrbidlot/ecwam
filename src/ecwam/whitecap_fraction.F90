...
SUBROUTINE WHITECAP_FRACTION (KIJS, KIJL, FL1, XLLWS, CINV, DEPTH, WSWAVE, UFRIC, COSWDIF, PHIOCD, ZWCF)
...
REAL(KIND=JWRB), DIMENSION(KIJL) :: ZPWVAGE_U10, ZPWVAGE_US, EMEAN
REAL(KIND=JWRB), DIMENSION(KIJL) :: ZPWVAGE_U10_SEA, ZPWVAGE_US_SEA, ESEA, ZHSWW
...
CALL SEMEAN(FL1, KIJS, KIJL, EMEAN, .TRUE.)
! WAVEAGE
...
! WAVEAGE of the windsea
CALL PEAK_WAVEAGE (KIJS, KIJL, F1, DEPTH, WSWAVE, UFRIC, ZPWVAGE_U10_SEA, ZPWVAGE_US_SEA)

! SELECT RELEVANT WAVEAGE
DO IJ=KIJS,KIJL
   ! THERE IS SOME WINDSEA
   IF( ESEA(IJ)/EMEAN(IJ) > 0.25_JWRB) THEN
     ZPWVAGE_U10(IJ)=MIN(ZPWVAGE_U10_SEA(IJ),ZPWVAGE_U10(IJ))
   ENDIF
ENDDO
...
DO IJ=KIJS,KIJL
  ...
  IF ( ZPWVAGE_U10(IJ) <= OLDWSFC ) THEN
    ZX = ZPWVAGE_U10(IJ)**2
  ELSE
    ZX = (OLDWSFC**2/ZPWVAGE_U10(IJ))**2
  ENDIF

!!!  IF ( ZPWVAGE_US(IJ) <= ZPMLIM ) THEN
!!!    ZX = ZFRICM2 * ZPWVAGE_US(IJ)**2
!!!  ELSE
!!!    ZX = ZFRICM2 * (ZPMLIM**2/ZPWVAGE_US(IJ))**2
!!!  ENDIF

  ZWCF(IJ) = MIN(MAX((-PHIOCD(IJ)*ZX - PPHITH)*(1.0_JWRB+ZD)/(ZOMRHO*ZP), 0.0_JWRB),1.0_JWRB)
ENDDO
