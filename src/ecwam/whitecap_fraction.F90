...
SUBROUTINE WHITECAP_FRACTION (KIJS, KIJL, FL1, XLLWS, CINV, DEPTH, WSWAVE, CICOVER, UFRIC, COSWDIF, PHIOCD, ZWCF)
...
REAL(KIND=JWRB) :: ZPMLIM, ZFRICM2, ZOMRHO, ZD, ZP, ZX, ZX_SEA
...
DO IJ=KIJS,KIJL
  ...
  ENDIF
  IF ( ZPWVAGE_U10_SEA(IJ) <= OLDWSFC ) THEN
    ZX_SEA = ZPWVAGE_U10_SEA(IJ)**2
  ELSE
    ZX_SEA = (OLDWSFC**2/ZPWVAGE_U10_SEA(IJ))**2
  ENDIF

  ZX = MAX(ZX, ZX_SEA)

  ZWCF(IJ) = (1.0_JWRB - CICOVER(IJ)) * MIN(MAX((-PHIOCD(IJ)*ZX - PPHITH)*(1.0_JWRB+ZD)/(ZOMRHO*ZP), 0.0_JWRB),1.0_JWRB)
ENDDO
