...
!-----------------------------------------------------------------------
     ...
     &            LRALTPREPROC, LRALTASCII, LRALTAVERAGE
...
9001 CONTINUE
  ...
  ELSE
    IF (LRALTASCII) THEN
      WRITE(IU06,*) '     GRFIELD :  ALTIMETER DATA ARE READ FROM AN ASCII FILE'
    ELSE
      WRITE(IU06,*) '     GRFIELD :  ALTIMETER DATA ARE READ FROM A BINARY FILE AS PREPRARED BY URAPRE'
    ENDIF
    IF (LRALTAVERAGE) THEN
      WRITE(IU06,*) '     GRFIELD :  CONTAINING ALONG TRACK AVERAGED OBSERVATIONS'
    ELSE
      WRITE(IU06,*) '     GRFIELD :  CONTAINING RAW OBSERVATIONS'
    ENDIF
    WRITE(IU06,*) ''
!!
    IF (IRANK == IREAD) THEN
      CALL GRDATA (NOBS, IDELALT, LRALTAVERAGE, NIJALTP1, LLRAWASCII=LRALTASCII)
      WRITE(IU06,*) ''
    ENDIF
  ENDIF
