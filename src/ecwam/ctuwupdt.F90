...
USE YOWCURR  , ONLY : LLCFLCUROFF, CURRENT_MAX, MRVLCUR
...
INTEGER(KIND=JWIM) :: IJ, K, M, J, KM1, KP1, IJ_EXT
...
MRVLCUR = NFRE_RED+1
ZCURMAX=SQRT(2.0_JWRB)*CURRENT_MAX
IF (IREFRA == 2 .OR. IREFRA == 3) THEN
  DO M = NFRE_RED,1,-1
    DO IJ_EXT = NINF,NSUP
      IF (CGROUP_EXT(IJ_EXT,M) < ZCURMAX) THEN
        MRVLCUR=M
        EXIT
      ENDIF
    ENDDO
