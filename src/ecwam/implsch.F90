...
SUBROUTINE IMPLSCH (KIJS, KIJL, FL1,                         &
 ...
 &                  NEMOWSWAVE, NEMOPHIF, &
 &                  WCF, TLEMEAN, MSS, MSSXX, MSSYY, MSSXY, &
...
! ----------------------------------------------------------------------
 ...
 &                             INTGT_PARAM_FIELDS, WAVE2OCEAN
      ...
      USE YOWCOUP  , ONLY : LWFLUX   , LWWCF, LWVFLX_SNL , LWNEMOCOU,    &
 &                          LWMSS,                                       &
...
#include "wnfluxes.intfb.h"
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL), INTENT(INOUT) :: MSS, MSSXX, MSSYY, MSSXY
...
!         CHECK THAT EMEAN(IJ) >= WSEMEAN(IJ)
      ...
      IF (LWWCF .AND. LCFLX) THEN
        CALL WHITECAP_FRACTION (KIJS, KIJL, FL1, XLLWS, CINV, DEPTH, WSWAVE, CICOVER, UFRIC, COSWDIF, PHIOCD, WCF)
      ENDIF

      IF (LWMSS) THEN
        ! NOTES: SET XKMSS (FIRST ARG.) TO ZERO SO THAT NO TAIL OR GC CONTRIBUTIONS ADDED
        CALL MEANSQS  (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, UFRIC, COSWDIF, MSS)
        CALL DMEANSQS (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM,                 MSSXX, MSSYY, MSSXY)
      ENDIF
