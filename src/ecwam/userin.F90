...
! ----------------------------------------------------------------------
     ...
     &            LWAM_USE_IO_SERV
      ...
      USE YOWCURR  , ONLY : IDELCUR  ,CDATECURA, LLCFLCUROFF, CURRENT_MAX, MRVLCUR
...
!!!! PROPAGS2 has not yet been adapted for anything else then
      ...
      ELSEIF (IREFRA == 2) THEN
        ...
        WRITE(IU06,*) ' THE MAXIMUM CURRENT SPEED ALLOWED IS ',SQRT(2.0_JWRB)*CURRENT_MAX
        WRITE(IU06,*) ' AND THE LOWEST FREQUENCY INDEX FOR WHICH PROPAGATION DIRECTION MIGHT REVERSED IS ',MRVLCUR
...
!     ICE WATER DRAG COEFFICIENT
     ...
     &                    CITHRSH_TAIL
      ...
      ENDIF
      IF (LICETH .OR. LWNEMOCOUCIT) THEN
        IF (LCIRSCTWC) WRITE (IU06,*) ' LINEAR SCALING OF SEA ICE THICKNESS BY SEA ICE COVER'
      ENDIF
