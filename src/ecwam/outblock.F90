...
! MEAN SQUARE SLOPE CALCULATED WITH NO CUT-OFF (I.E. NO TAIL OR GC CONTRIBUTIONS)
      IF (IPFGTBL(67 + 3*NTRAIN + NTEWH) /= 0) THEN
        CALL MEANSQS (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, UFRIC, COSWDIF, BOUT(:,ITOBOUT(67 + 3*NTRAIN + NTEWH)))
      ENDIF

! MSSXX
      IF (IPFGTBL(68 + 3*NTRAIN + NTEWH) /= 0) THEN
        CALL DMEANSQS (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, BOUT(:,ITOBOUT(68 + 3*NTRAIN + NTEWH)), MSSYY, MSSXY)
      ENDIF

! MSSYY
      IF (IPFGTBL(69 + 3*NTRAIN + NTEWH) /= 0) THEN
        CALL DMEANSQS (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, MSSXX, BOUT(:,ITOBOUT(69 + 3*NTRAIN + NTEWH)), MSSXY)
      ENDIF

! MSSXY
      IF (IPFGTBL(70 + 3*NTRAIN + NTEWH) /= 0) THEN
        CALL DMEANSQS (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, MSSXX, MSSYY, BOUT(:,ITOBOUT(70 + 3*NTRAIN + NTEWH)))
      ENDIF
