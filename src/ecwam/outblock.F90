...
#include "w_maxh.intfb.h"
      ...
      REAL(KIND=JWRB), DIMENSION(KIJL) :: USTD, VSTD
...
&         IPFGTBL(65 + 3*NTRAIN + NTEWH) /= 0 .OR. IPFGTBL(66 + 3*NTRAIN + NTEWH) /= 0 ) THEN
     ...
     &               CMAX_F, HMAX_N, CMAX_ST, HMAX_ST, PHIST)
      ...
      DO ISTK = 1, NTSTK
        IF (IPFGTBL(71 + 3*NTRAIN + NTEWH + 2*ISTK-1) /= 0 .OR. IPFGTBL(71 + 3*NTRAIN + NTEWH + 2*ISTK) /= 0) THEN
          CALL STKDRFWD(KIJS, KIJL, FL1, DPTHSTK(ISTK), WAVNUM, STOKFAC, DEPTH, WSWAVE, WDWAVE, CICOVER, USTD, VSTD)
        ENDIF

        IF (IPFGTBL(71 + 3*NTRAIN + NTEWH + 2*ISTK-1) /= 0) THEN
          BOUT(KIJS:KIJL,ITOBOUT(71 + 3*NTRAIN + NTEWH + 2*ISTK-1))=USTD(KIJS:KIJL)
        ENDIF

        IF (IPFGTBL(71 + 3*NTRAIN + NTEWH + 2*ISTK)   /= 0) THEN
          BOUT(KIJS:KIJL,ITOBOUT(71 + 3*NTRAIN + NTEWH + 2*ISTK))=VSTD(KIJS:KIJL)
        ENDIF
      ENDDO
...
! MEAN SQUARE SLOPE CALCULATED WITH NO CUT-OFF (I.E. NO TAIL OR GC CONTRIBUTIONS)
      IF (IPFGTBL(72 + 3*NTRAIN + NTEWH + 2*NTSTK) /= 0) THEN
        CALL MEANSQS (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, UFRIC, WSWAVE, COSWDIF, BOUT(:,ITOBOUT(72 + 3*NTRAIN + NTEWH + 2*NTSTK)))
      ENDIF

! MSSXX
      IF (IPFGTBL(73 + 3*NTRAIN + NTEWH + 2*NTSTK) /= 0) THEN
        CALL DMEANSQS_LF (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, BOUT(:,ITOBOUT(73 + 3*NTRAIN + NTEWH + 2*NTSTK)), MSSYY, MSSXY)
      ENDIF

! MSSYY
      IF (IPFGTBL(74 + 3*NTRAIN + NTEWH + 2*NTSTK) /= 0) THEN
        CALL DMEANSQS_LF (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, MSSXX, BOUT(:,ITOBOUT(74 + 3*NTRAIN + NTEWH + 2*NTSTK)), MSSXY)
      ENDIF

! MSSXY
      IF (IPFGTBL(75 + 3*NTRAIN + NTEWH + 2*NTSTK) /= 0) THEN
        CALL DMEANSQS_LF (0.0_JWRB, KIJS, KIJL, FL1, WAVNUM, MSSXX, MSSYY, BOUT(:,ITOBOUT(75 + 3*NTRAIN + NTEWH + 2*NTSTK)))
      ENDIF

! xxx 
      IF (IPFGTBL(76 + 3*NTRAIN + NTEWH + 2*NTSTK) /= 0) THEN
         BOUT(:,ITOBOUT(76 + 3*NTRAIN + NTEWH + 2*NTSTK)) = 0.0_JWRB
      ENDIF
