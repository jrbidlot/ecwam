...
! -------------------------------------------------------------------
     ...
     &         LIFS_IO_SERV_ENABLED
      ...
      USE YOWMAP   , ONLY : BLK2GLO  ,BLK2LOC  ,ZDELLO
...
!     IFS IO SERVER ENABLED
      ...
      REAL(KIND=JWRB), DIMENSION(NPROMA_WAM, NCHNK) :: ZPWVAGE_U10, ZPWVAGE_US, ZMEANSS
...
!       UPDATE ANALYSIS TIME
 ...
 &                        WAM2NEMO, NEMO2WAM, VARS_4D%FL1)
          ENDIF
        ELSEIF ( (.NOT.LWCOU .AND. CDTPRO <= CDATEF .AND. CDTPRO == CDTASS ) .OR. (LWCOU .AND. CDTPRO == CDATEF) ) THEN
          CALL WAMASSI (LDSTOP, LDWRRE, BLK2GLO,          &
 &                      WVENVI, WVPRPT, FF_NOW, INTFLDS,  &
 &                      WAM2NEMO, NEMO2WAM, VARS_4D%FL1)
          CALL INCDATE(CDTASS, IDELALT)
...
!!!!    The order is important see arpifs/setup/su_surf_flds.F90 !!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!       1. CHARNOCK FIELD(S)
        FLABEL(1)=' Charnock'
        DEFVAL(1)=PRCHAR ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY THE MODEL
        IFLDOFFSET=1

        IF (LWCOUHMF) THEN
!         2. REDUCED CHARNOCK FOR HEAT AND MOISTURE EXCHANGE 
          IFLDOFFSET=IFLDOFFSET+1
          FLABEL(IFLDOFFSET)=' Eqv Chnk'
          DEFVAL(IFLDOFFSET)=PRCHAR
        ENDIF
...
!         3. U-STOKESDRIFT
          ...
          DEFVAL(IFLDOFFSET)=0.0_JWRB  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY THE MODEL
...
!         5. ENERGY FLUX TO OCEAN (dimensional)
          IFLDOFFSET=IFLDOFFSET+1
...
!         7. X-COMPONENT OF THE MOMENTUM FLUX TO OCEAN (dimensional)
          IFLDOFFSET=IFLDOFFSET+1
...
!         8. WINDSEA VARIANCE
          ...
          DEFVAL(IFLDOFFSET)=FLMIN  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY THE MODEL

!         9. WINDSEA MEAN FREQUENCY
          IFLDOFFSET=IFLDOFFSET+1
          FLABEL(IFLDOFFSET)=' WSFmean'
          DEFVAL(IFLDOFFSET)=FR(NFRE)  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY THE MODEL
...
!         11. WAVE AGE AT THE SPECTRAL PEAK (U10 based, see below) 
          IFLDOFFSET=IFLDOFFSET+1
          FLABEL(IFLDOFFSET)='Wave_age'
          DEFVAL(IFLDOFFSET)=G/ZPIFR(NFRE)  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY THE MODEL

!         12. MEAN SQUARE SLOPE
          IFLDOFFSET=IFLDOFFSET+1
          FLABEL(IFLDOFFSET)='  MeanSS'
          DEFVAL(IFLDOFFSET)=FLMIN  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY THE MODEL

        ENDIF

        IF (LWWCF) THEN
!         13. WHITECAP FRACTION
          IFLDOFFSET=IFLDOFFSET+1
          FLABEL(IFLDOFFSET)=' Whcapfr'
          DEFVAL(IFLDOFFSET)=0.0_JWRB  ! DEFAULT VALUE FOR GRID POINTS NOT COVERED BY THE MODEL
        ENDIF
...
!         IT MIGHT ALSO BE USED FOR NEMO !!!!!!!!!!
          ...
          IF (LWFLUX .OR. LWSPRAY) THEN
            ...
            CALL WVFLD2IFS (1, NPROMA_WAM,  &
     &                      VARS_4D%FL1(:,:,:,ICHNK), WVENVI%DEPTH(:,ICHNK), WVPRPT%WAVNUM(:,:,ICHNK), &
     &                      FF_NOW%WSWAVE(:,ICHNK), FF_NOW%WDWAVE(:,ICHNK), FF_NOW%UFRIC(:,ICHNK),     &
     &                      ZPWVAGE_U10(:,ICHNK), ZPWVAGE_US(:,ICHNK), ZMEANSS(:,ICHNK) )

            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(IJSB:IJLB,IFLDOFFSET)=INTFLDS%PHIOCD(KIJS:KIJL,ICHNK)  !!! defined negative !!!!
            IFLDOFFSET=IFLDOFFSET+1
            ...
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(IJSB:IJLB,IFLDOFFSET)=INTFLDS%TLEMEAN(KIJS:KIJL,ICHNK)
            IFLDOFFSET=IFLDOFFSET+1
!!!!!! JB_note : passing the wave age based on U10 !!!!
            WVBLOCK(IJSB:IJLB,IFLDOFFSET)=ZPWVAGE_U10(KIJS:KIJL,ICHNK)
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(IJSB:IJLB,IFLDOFFSET)=ZMEANSS(KIJS:KIJL,ICHNK)

          ENDIF

          IF (LWWCF) THEN
            IFLDOFFSET=IFLDOFFSET+1
            WVBLOCK(IJSB:IJLB,IFLDOFFSET)=INTFLDS%WCF(KIJS:KIJL,ICHNK)
          ENDIF
...
!         COMPUTE PSEUDO GLOBAL NORM
          IF (IRANK == IRECV) THEN
            ...
            WRITE(IU06,*) ' For IRANK, NPROC, LL1D ', IRANK, NPROC, LL1D
            IST=1+(IRANK-1)*NCOMLOC
            ICOUNT=IST
