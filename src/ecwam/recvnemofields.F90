...
!     UPDATE CICOVER, CITHICK, IBRMEM, UCUR AND VCUR AT !!! INITIAL TIME ONLY !!!!
      IF (LINIT) THEN

        WRITE(IU06,*)' RECVNEMOFIELDS: INITIALISE OCEAN FIELDS'

!!temporary: the use of IPARAM will be remove in the contribution to 51R1
        IPARAM=31
...
!$OMP   PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(ICHNK, KIJS, KIJL)
        DO ICHNK = 1, NCHNK
          ...
          CALL MICEP(IPARAM, KIJS, KIJL, BLK2LOC%IFROMIJ(:,ICHNK), BLK2LOC%JFROMIJ(:,ICHNK),  &
     &               NXS, NXE, NYS, NYE, FIELDG,                                      &
     &               FF_NOW%CICOVER(:,ICHNK), FF_NOW%CITHICK(:,ICHNK),                &
     &               NEMO2WAM%NEMOCICOVER(:,ICHNK), NEMO2WAM%NEMOCITHICK(:,ICHNK),    &
     &               WVENVI%IBRMEM(:,ICHNK), NEMO2WAM%NEMOCIIBR(:,ICHNK))
