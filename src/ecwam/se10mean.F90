...
! ----------------------------------------------------------------------
      ...
      FMIDPT = 0.5_JWRB * (FR(MCUT) + FR(MCUT-1))
      FSTAR = 0.5_JWRB * ( FCUT + FMIDPT)
      DELF = FR(MCUT) - FR(MCUT-1)
      WL = (FR(MCUT) - FSTAR) /DELF
      WR = 1.0_JWRB - WL

      DFIMLOC(MCUT-1) = DFIMLOC(MCUT-1) + DELTH * (FCUT-FMIDPT) * WL
      DFIMLOC(MCUT) = DELTH * (FCUT-FMIDPT) * WR 
