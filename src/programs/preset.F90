...
!!!!! !$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) PRIVATE(ICHNK,KIJS,KIJL,IJSB,IJLB,M,K,FLCHNK)
        DO ICHNK = 1, NCHNK
...
!         reset points with no flux out of the boundary to 0
            ...
            DO M=1,NFRE
              DO K=1,NANG
                FL(KIJS:KIJL,K,M) = FLCHNK(KIJS:KIJL,K,M) * IOBPD(K,KIJS:KIJL)
              ENDDO
            ENDDO
#endif
          ENDIF
