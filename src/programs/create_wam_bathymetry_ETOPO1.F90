...
!     IF RATIOLAND_THRESHOLD OR MORE LAND OR THE CENTER OF THE GRID BOX IS LAND, THEN AVERAGE OVER LAND POINTS
      ...
      REAL(KIND=JWRU) ::  RATIOLAND_OBSTR
...
!!    PENHCOR is used to adjust the corner obstructructions for IPROPAGS=2
      ...
      INTEGER(KIND=JWIM) :: IU01, IU06, IUGRD, IUNIT, IUSGOBS
...
!       PERCENTAGE OF SUBGRID LAND AND OBSTRUCTIONS AWAY FROM COASTLINES
!       --------------------------------------------
        IF ( M == 1 ) THEN
          LLSGRDOBS=.TRUE.
...
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1) &
!$OMP& PRIVATE(K,XLATT,XLATB,ILATT,ILATB,IX) &
!$OMP& PRIVATE(XLONL,XLONR,ILONL,ILONR,NOBSTRCT,NLAND) &
...
!                 RATIO OF ALL BLOCKING SUBGRID POINTS TO THE TOTAL NUMBER OF POINTS
                  IF ( REAL(NOBSTRCT,JWRU)/REAL(NTOTPTS,JWRU) < RATIOLAND_OBSTR ) THEN
                    SGRDOBS(IX,K) = REAL(NLAND,JWRU)/REAL(NTOTPTS,JWRU)
                  ELSE
                    SGRDOBS(IX,K) = 0.0_JWRU
                  ENDIF
